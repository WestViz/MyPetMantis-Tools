...
import { 
    ...
    ThermometerSun, AlertTriangle, Leaf, Bug
} from 'lucide-react';

type AgeGroup = 'baby' | 'juvenile' | 'adult';
type ActivityLevel = 'normal' | 'brumation' | 'gravid' | 'sick';
...
const AGE_PRESETS = {
    baby: { w: 20, label: 'Baby (0-4 mo)', freq: 3 },
    juvenile: { w: 200, label: 'Juvenile (4-12 mo)', freq: 2 },
    adult: { w: 400, label: 'Adult (12+ mo)', freq: 1 },
};

const ACTIVITY_PRESETS = {
    normal: { label: 'Normal', multiplier: 1.0, desc: 'Active & Healthy' },
    brumation: { label: 'Brumation', multiplier: 0.3, desc: 'Winter Sleep' },
    gravid: { label: 'Gravid', multiplier: 1.4, desc: 'Egg Laying' },
    sick: { label: 'Recovery', multiplier: 0.8, desc: 'Illness/Recovery' },
};
...
const BeardedDragonDietCalculator: React.FC = () => {
    ...
    useEffect(() => {
        calculateDiet();
    }, [inputs.weight, inputs.ageGroup, inputs.activityLevel, inputs.includeGreens, inputs.includeInsects, inputs.supplements]);

    const calculateDiet = () => {
        const { weight, ageGroup, activityLevel, includeGreens, includeInsects, supplements } = inputs;

        // Base calculations by age
        let baseInsects = 0;
        let baseGreens = 0;
        let feedingFreq = 1;
        ...
        switch (ageGroup) {
            case 'baby':
                baseInsects = Math.round(weight * 0.4); // 40% of body weight in insects
                baseGreens = Math.round(weight * 0.05); // Small amount of greens
                feedingFreq = 3; // 3x per day
                break;
            case 'juvenile':
                baseInsects = Math.round(weight * 0.25); // 25% insects
                baseGreens = Math.round(weight * 0.15); // 15% greens
                feedingFreq = 2; // 2x per day
                break;
            case 'adult':
                baseInsects = Math.round(weight * 0.1); // 10% insects
                baseGreens = Math.round(weight * 0.25); // 25% greens
                feedingFreq = 1; // 1x per day
                break;
        }

        // Apply activity multiplier
        const multiplier = ACTIVITY_PRESETS[activityLevel].multiplier;
        const adjustedInsects = Math.round(baseInsects * multiplier);
        const adjustedGreens = Math.round(baseGreens * multiplier);
        ...
        const supplementsPerWeek = supplements ? (ageGroup === 'baby' ? 7 : 3) : 0;

        // Warnings
        const warnings: string[] = [];
        if (weight < 10) warnings.push("⚠️ Very small dragon - consult a vet for feeding guidance.");
        if (weight > 700) warnings.push("🦎 Large dragon - monitor weight to prevent obesity.");
        if (ageGroup === 'adult' && proteinRatio > 30) warnings.push("📏 Adults should have 20-30% protein to prevent obesity.");
        if (activityLevel === 'brumation' && adjustedInsects > 0) warnings.push("💤 During brumation, insects should be minimal or stopped.");

        // Tips
        const tips: string[] = [];
        if (ageGroup === 'baby') {
            tips.push("Dust all insects with calcium powder at every feeding.");
            tips.push("Offer 10-15 minute feeding sessions, 3x daily.");
        } else if (ageGroup === 'adult') {
            tips.push("Adults need mostly greens - insects are treats now!");
            tips.push("Feed protein every other day, greens daily.");
        }
        if (activityLevel === 'gravid') {
            tips.push("Gravid females need extra calcium - dust insects heavily.");
        }
        ...
        <div className="glass-panel text-slate-900 rounded-[2rem] shadow-xl bg-white/95 backdrop-blur-md border border-brand-700/20 p-0 md:p-8 relative overflow-hidden max-w-6xl mx-auto font-sans">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-5 space-y-4">
                        <div className="bg-gradient-to-br from-brand-900 to-brand-800 text-white p-6 md:p-8 rounded-[2rem] shadow-2xl relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none"><Scale className="w-48 h-48" /></div>
                                ...
                                {result && (
                                    <>
                                        <div className="space-y-3 mb-6">
                                            ...
                                            <div className="bg-black/20 p-3 rounded-xl">
                                                <div className="flex justify-between text-xs mb-1">
                                                    <span className="text-brand-100 flex items-center gap-1"><Leaf className="w-3 h-3" /> Greens & Vegetables</span>
                            ...
                            <ul className="space-y-2">
                                {result?.tips.map((item, i) => (
                                    ...
                                    <li className="flex items-start gap-2 text-xs text-slate-600">
                                        <div className="w-1.5 h-1.5 rounded-full bg-brand-600 mt-1 shrink-0"></div>
                                        Select age and weight to see personalized tips.
                                    </li>
                                )}
                            </ul>
                        </div>
            ...
            <div className="text-center mt-6">
                <p className="text-brand-700 text-sm">
                    ...
                    Individual dragons may have unique requirements.
                </p>
            </div>
        </div>
    );
...
export default BeardedDragonDietCalculator;